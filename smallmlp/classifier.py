...
from .conformal import conformal_qhat_classification, tune_h_cal_classification
from .conformal import _to_tensor


class SmallMLPClassifier(BaseEstimator, ClassifierMixin):
    """Small-data binary classifier with weighted conformal prediction sets.

    Point prediction:
      - "nw":  NW vote (weighted mean of labels), bandwidth learned by LOO-BCE
      - "klr": Kernel Logistic Regression, alpha/b/h learned by LOO-BCE + L2

    Prediction sets: weighted conformal classification with finite-sample
    coverage guarantee. Same conformal layer for both point methods.
    """

    def __init__(self, point_method="nw", h_min=0.01, h_max=10.0,
                 lam=1e-3, class_weight=None,
                 max_iter=30, inner_iter=10, tol=1e-6, verbose=False,
                 alpha=1e-3):
    ...
    def fit(self, X, y):
        ...
        if self.point_method == "nw":
            params = [psi]
        elif self.point_method == "klr":
            ...
            raise ValueError(f"Unknown point_method: {self.point_method}")
        ...
        for it in range(self.max_iter):
            def closure():
                ...
                if self.point_method == "nw":
                    ...
                    loss = klr_loo_bce_loss(
                        self._alpha_param, self._b_param, psi, X_t, y_t,
            ...
            if self.verbose:
                tag = "SmallMLPClassifier" if self.point_method == "nw" else "KLR"
                print(f"[{tag}] iter {it:3d}  loss={loss_val:.6e}")
        ...
        if self.point_method == "klr":
            self._alpha = self._alpha_param.detach()
            self._b = self._b_param.detach()
    ...
    def _predict_p(self, X):
        """Return P(y = class 1) as numpy array."""
        X_t = self._prepare_query(X)
        with torch.no_grad():
            h = self._h_from_psi(self._psi)
            if self.point_method == "nw":
                p_hat, _, _ = forward(
                    X_t, self._X_train, self._y_train, h, alpha=self.alpha
                )
            else:  # klr
                p_hat = klr_forward(
                    X_t, self._X_train, self._alpha, self._b, h
                )
        return p_hat.numpy()
    ...
    def fit_conformal(self, X_cal, y_cal, X_val, y_val,
        ...
        """Tune h_cal on validation, store calibration scores."""
        check_is_fitted(self, ["_psi", "_X_train", "_y_train"])
    ...
    def predict_set(self, X, alpha=None):
        """Return prediction sets as list of sets."""
